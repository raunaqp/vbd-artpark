...
function ResponseTabContent() {
  ...
  /**
   * Summarise one table row as the ward-grain area aggregate both modals take.
   *
   ...
   *
   * Shared by both actions so a ward cannot be described one way by Log
   * Response and another by No Activity.
   */
  const wardAggregate = useCallback(
    (r: PriorityRow) => {
      ...
      return summarizeRow(row, weekRecords);
    },
    [stateId, weekRecords],
  );

  const handleLog = useCallback((r: PriorityRow) => openDrawer(wardAggregate(r)), [openDrawer, wardAggregate]);

  // Restores the "no field activity this week" workflow. Deleting
  // AreaResponseTable in R4.4.4 removed its only entry point, leaving
  // NoActivityDialog mounted but unreachable.
  const handleNoActivity = useCallback(
    (r: PriorityRow) => openNoActivity(wardAggregate(r)),
