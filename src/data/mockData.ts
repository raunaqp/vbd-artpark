// Mock data for Vector-Borne Disease EWS Dashboard — Hierarchical, Multi-State
// Three states: Andhra Pradesh (reference), Odisha, Karnataka — identical structure, different geography/data.
// All data is partitioned by state. The active state is set by StateContext via setActiveState(id).
...
export interface StateBundle {
  id: StateId;
  label: string;
  districts: string[];                      // includes "All Districts"
  blocks: string[];                         // includes "All Blocks"
  regionData: RegionData[];                 // district-level
  subDistrictData: RegionData[];            // blocks + municipalities
...
// ──────────────── ANDHRA PRADESH (reference standard) ────────────────
const AP: StateBundle = {
  ...
  mapZoom: 7,
  districtCoordinates: {
    "East Godavari": [17.0, 81.8], "Krishna": [16.6, 80.6], "S.P.S. Nellore": [14.4, 79.9],
    "Prakasam": [15.3, 79.4], "Srikakulam": [18.3, 84.0], "Visakhapatnam": [17.7, 83.3],
    "Vizianagaram": [18.1, 83.4], "West Godavari": [16.9, 81.2], "Guntur": [16.3, 80.4],
    "Kurnool": [15.8, 78.0], "Anakapalli": [17.7, 83.0], "Eluru": [16.7, 81.1],
    "Tenali": [16.24, 80.64], "Mangalagiri": [16.43, 80.57], "Amaravathi": [16.57, 80.36],
    "Pedakakani": [16.25, 80.48], "Guntur West": [16.31, 80.38], "Bheemunipatnam": [17.89, 83.45],
    "Vizag MC": [17.72, 83.30], "Gajuwaka": [17.70, 83.22], "Pendurthi": [17.76, 83.21],
    "Anakapalle": [17.69, 83.00], "Nandyal": [15.48, 78.48], "Adoni": [15.63, 77.28],
    "Kallur": [15.69, 78.05], "C.Belagal": [15.70, 78.15], "Vijayawada MC": [16.51, 80.65],
    "Machilipatnam": [16.19, 81.14], "Gudivada": [16.44, 81.10], "Nuzvid": [16.79, 80.85],
    "Kollipara": [16.22, 80.62], "Pedaravuru": [16.26, 80.66], "Angalakuduru": [16.23, 80.60],
    "Duggirala": [16.32, 80.58], "Thagarapuvalasa": [17.91, 83.47], "Anandapuram": [17.87, 83.43],
    "Rushikonda": [17.78, 83.38], "Kapuluppada": [17.85, 83.45], "Kasimkota": [17.66, 82.97],
    "Chodavaram": [17.83, 82.93], "Mahanandi": [15.58, 78.62], "Banaganapalle": [15.68, 78.23],
    "Koilkuntla": [15.23, 78.32], "Undavalli": [16.50, 80.44], "Tadepalli": [16.47, 80.62],
    "Penumaka": [16.52, 80.38], "Pedagantyada": [17.75, 83.25], "Nathayyapalem": [17.73, 83.20],
    "Aganampudi": [17.68, 83.18], "MVP Colony": [17.74, 83.32], "Dwaraka Nagar": [17.73, 83.26],
    "Akkayyapalem": [17.70, 83.34], "Seethammadhara": [17.72, 83.30], "Benz Circle": [16.53, 80.67],
    "Governorpet": [16.50, 80.63], "Patamata": [16.49, 80.61], "Gunadala": [16.52, 80.65],
...
const ODISHA: StateBundle = {
  ...
  subDistrictData: [
    ...
    // Puri — low cases, high forecast (mobility)
  ...
  villageData: [
    ...
    // Brahmagiri (Puri) — coastal villages
  ...
  wardData: [
    ...
    // Rourkela MC — industrial worker clustering
  ...
  statePredictions: [
    ...
    { area: "Sundargarh", probability: 72, risk: "high", expectedWeek: "W+2", signal: "Industrial worker clustering in Rourkela (Chhend, Panposh) — localized spikes" },
...
export const stateBundles: Record<StateId, StateBundle> = {
  andhra_pradesh: AP,
  odisha: ODISHA,
  karnataka: KARNATAKA,
};
...
export function getActiveStateId(): StateId {
  return activeStateId;
}

export function subscribeStateChange(fn: () => void): () => void {
...
// ──────────────── Backwards-compatible exports (computed via getters) ────────────────
// We expose accessor functions that internally read the active state.
export const getDistricts = () => S().districts;
export const getBlocks = () => S().blocks;
export const getMapCenter = (): [number, number] => S().mapCenter;
export const getMapZoom = (): number => S().mapZoom;

// Legacy names — many components import these. Keep them as objects with getters.
export const districts: string[] = new Proxy([] as string[], {
  get(_t, prop) { return Reflect.get(S().districts, prop); },
  has(_t, prop) { return Reflect.has(S().districts, prop); },
  ownKeys() { return Reflect.ownKeys(S().districts); },
  getOwnPropertyDescriptor(_t, prop) { return Reflect.getOwnPropertyDescriptor(S().districts, prop); },
}) as string[];

export const blocks: string[] = new Proxy([] as string[], {
  get(_t, prop) { return Reflect.get(S().blocks, prop); },
  has(_t, prop) { return Reflect.has(S().blocks, prop); },
  ownKeys() { return Reflect.ownKeys(S().blocks); },
  getOwnPropertyDescriptor(_t, prop) { return Reflect.getOwnPropertyDescriptor(S().blocks, prop); },
}) as string[];

export const wards = ["All Wards"];

// Live data accessors (used by getFiltered* functions). Re-export as Proxy arrays for legacy imports.
...
// districtCoordinates — Proxy object so lookups always hit the active state
export const districtCoordinates: Record<string, [number, number]> = new Proxy({} as Record<string, [number, number]>, {
  ...
  getOwnPropertyDescriptor(_t, prop: string) {
    if (prop in S().districtCoordinates) {
      return { enumerable: true, configurable: true, value: S().districtCoordinates[prop] };
    }
    return undefined;
  },
});

// mapCenter / mapZoom — exported as live getters via Object.defineProperty pattern won't work for primitives,
// so consumers should use getMapCenter/getMapZoom OR import districtCoordinates directly. We keep these as initial AP defaults
// for any leftover reference; DashboardMap uses getMapCenter/getMapZoom via re-exports below.
export const mapCenter: [number, number] = AP.mapCenter; // initial fallback only
export const mapZoom: number = AP.mapZoom;

// ──────────────── Filter functions (state-aware) ────────────────
...
export function getOutbreakPredictions(district: string, block?: string): OutbreakPrediction[] {
  ...
  if (district && district !== "All Districts") {
    return s.districtPredictions.filter((p) => p.parentDistrict === district).sort((a, b) => b.probability - a.probability);
  }
